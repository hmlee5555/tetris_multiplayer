class ConnectionManager{
    ...
    initSession(){
        ...
        if (sessionId){ // 주소 뒤에 세션ID 있으면 그 세션 join
            this.send({
                type: 'join-session',
                id: sessionId,
                state,
            });
        }else{ // 세션ID 없으면 세션 새로 생성
            this.send({
                type: 'create-session',
                state,
            });
    ...
    watchEvents(){
        ...
        ['pos','score','matrix','nextPiece','savedPiece','time','speed'].forEach(prop => {
    ...
    updateManager(peers){       // 여기서 peers는 broadcastSession으로 전달받는 peers객체 (this.peers랑 다름)
        ...
        [...this.peers.entries()].forEach(([id,tetris]) => {
            if (!clients.some(client => client.id === id)) { // clients에서 loop하며 id와 동일한거 찾기: 없으면 삭제
        ...
        // sorting
        const sorted = peers.clients.map(client => {        // peers에 나타는 순서대로 tetris 객체들의 array만듦
    ...
    updatePeer(id, fragment, [prop, value]){
        if (!this.peers.has(id)){   // peer중에 해당 id 없는 경우
